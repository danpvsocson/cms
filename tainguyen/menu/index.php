<?php
    include('tainguyen/menu/xoa.php');
...
<section>
    <div class="container-fluid">
        <h2 class="float-start fs-4">Danh Sách Menu</h2>
        <a href="?thumuc=<?=$_GET['thumuc']?>&file=themmoi" class="float-end mt-2 mb-3 me-3 text-decoration-none btn btn-primary"><i class="bi bi-plus-lg"></i> Thêm Menu</a>
                ...
                <?php
                    ...
                    foreach ($view_all as $key => $info) {
                        echo '<tr>
                                <td>' . ($i++) . '</td>
                                <td>' . $info['tieude'] . '</td>';
                                $sql1 = "SELECT tieude FROM b_menu where id=:id_cha";
                                $a = $conn->prepare($sql1);
                                $data = array("id_cha" => $info['id_cha']);
                                $a->execute($data);
                                $view = $a->fetch();
                                    if($view){
                                        echo'<td>' .$view['tieude'] . '</td>';}
                                    else{
                                        echo'<td> </td>';
                                    }
                            echo'<td>' . $info['tieudemodul'] . '</td>
                            <td>' . $info['nhom'] . '</td>
                                <td>' . $info['ghichu'] . '</td>';
                        if ($info['hienthi'] == 1) {
