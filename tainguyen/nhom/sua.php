<section class="container">
    ...
    if ($flagcheck) {
        if (isset($_POST['btnluu'])) {
            ...
            if (!$error) {
                //Sủa dữ liệu
                $sql1 = "UPDATE nhom SET ma=:ma,tieude=:tieude,modul=:modul,hienthi=:hienthi where id=:id";
                $q = $conn->prepare($sql1);
                $data1 = array(
                    "ma" => $ma,
                    "tieude" => $tieude,
    ...
    //Lấy dữ liệu theo ID
    $sql = "SELECT * FROM nhom WHERE id=:id";
    $q = $conn->prepare($sql);
    $data = array("id" => $id);
    $q->execute($data);
    $view = $q->fetch();
        ...
        <div class="col-12">
            <div class="row">
                ...
                <div class="col-md-4">
                    ...
                    <input type="text" class="form-control" name="txt_ma" value="<?= $view['ma'] ?>" placeholder="Nhập mã">
                </div>
