<?php
    include('tainguyen/nhom/xoa.php');
    //Lấy dữ liệu từ database
    //Liên kết content <=> modul
    $sql="SELECT nhom.*,
            b_modul.tieude as tieudemodul
        FROM nhom 
            left outer join b_modul on b_modul.ma=nhom.modul
        ";
    $q=$conn->prepare($sql);
    $q->execute();
    $view_all = $q->fetchAll();
...
<section>
    <div class="container-fluid">
        <h2 class="float-start fs-4">Danh Sách Nhóm</h2>
        <a href="?thumuc=<?=$_GET['thumuc']?>&file=themmoi" class="float-end mt-2 mb-3 me-3 text-decoration-none btn btn-primary"><i class="bi bi-plus-lg"></i> Thêm Nhóm</a>
        ...
        <nav aria-label="Page navigation example" style="margin-left: 49%;">
            <ul class="pagination m-0">
                ...
                <?php
                $q = $conn->prepare("SELECT * from nhom order by id ASC");
                $q->execute();
                $rc = $q->rowCount();
                $tong_page = 0;
                if ($rc % $ban_ghi == 0) {
