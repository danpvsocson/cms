<?php
    include('tainguyen/'.$_GET['thumuc'].'/xoagroup.php');
...
<section>
    <div class="container-fluid">
        <h2 class="float-start fs-4">Danh Sách Nhóm Bài Viết</h2>
        <a href="?thumuc=<?=$_GET['thumuc']?>&file=addgroup" class="float-end mt-2 mb-3 me-3 text-decoration-none btn btn-primary"><i class="bi bi-plus-lg"></i> Thêm Nhóm Bài Viết</a>
            ...
            <tbody>
                ...
                foreach ($view_all as $key => $info) {
                    echo '<tr>
                            ...
                            <td>' . $info['tieu_de'] . '</td>
                            <td>' . $info['ma'] . '</td>
                            <td>' . $info['so_bai_viet'] . '</td>
                            <td class="text-center"><a href="?thumuc='.$_GET['thumuc'].'&file=suagroup&id=' . $info['id'] . '" class="btn btn-primary btn-sm">Sửa</a></td>
        ...
        <nav aria-label="Page navigation example" style="margin-left: 49%;">
            <ul class="pagination m-0">
                ...
                <?php
                $q = $conn->prepare("SELECT * from nhom_content order by id ASC");
                $q->execute();
                $rc = $q->rowCount();
                $tong_page = 0;
                if ($rc % $ban_ghi == 0) {
